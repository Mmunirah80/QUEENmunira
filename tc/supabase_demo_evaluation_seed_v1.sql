-- =============================================================================
-- DEMO EVALUATION (stakeholder emails @naham.com / @naham.demo) — derived from QA seed — FULL seed: cook documents matrix, restrictions, orders, chats
-- =============================================================================
-- [عربي] إذا «ما يضبط»:
...
--   Section 0 deletes auth.identities + auth.users for this seed’s emails when id ≠ fixed UUID (then section 1 inserts).
--   Auth loop still skips duplicate id/email (safe re-run).
-- =============================================================================
...
--
-- Inspect in Admin: filter profiles by full_name starting with "QA " or email @naham.demo / admin2@naham.com
-- =============================================================================
...
--    If admin2@naham.com existed with another id, we delete that user so section 1 can insert a001.
--    Disposable DB only. Run identities → users.
-- ═══════════════════════════════════════════════════════════════════════════
DO $$
DECLARE
  v_expected uuid[];
BEGIN
  ...
  WHERE user_id IN (
    ...
    WHERE NOT (u.id = ANY (v_expected))
      AND (
        lower(trim(u.email)) = 'admin2@naham.com'
        OR lower(trim(u.email)) IN (
          'customer1@naham.demo',
          'customer2@naham.demo',
          'customer3@naham.demo',
          'customer4@naham.demo',
          'customer5@naham.demo',
          'customer6@naham.demo',
          'customer7@naham.demo',
          'customer8@naham.demo'
        )
        OR lower(trim(u.email)) IN (
          SELECT format('cook%s@naham.demo', n) FROM generate_series(1, 25) AS n
        )
      )
  );
...
BEGIN
  ...
  FOR r IN
    SELECT * FROM (VALUES
      ('e0a00001-0000-4000-8a00-00000000a001'::uuid, 'admin2@naham.com'),
      ('e0a00001-0000-4000-8a00-00000000c001'::uuid, 'customer1@naham.demo'),
      ('e0a00001-0000-4000-8a00-00000000c002'::uuid, 'customer2@naham.demo'),
      ('e0a00001-0000-4000-8a00-00000000c003'::uuid, 'customer3@naham.demo'),
      ('e0a00001-0000-4000-8a00-00000000c004'::uuid, 'customer4@naham.demo'),
      ('e0a00001-0000-4000-8a00-00000000c005'::uuid, 'customer5@naham.demo'),
      ('e0a00001-0000-4000-8a00-00000000c006'::uuid, 'customer6@naham.demo'),
      ('e0a00001-0000-4000-8a00-00000000c007'::uuid, 'customer7@naham.demo'),
      ('e0a00001-0000-4000-8a00-00000000c008'::uuid, 'customer8@naham.demo')
    ) AS t(uid, em)
  LOOP
  ...
  FOR d IN 1..25 LOOP
    ...
    v_email := format('cook%s@naham.demo', d::text);
    IF EXISTS (
      SELECT 1 FROM auth.users
...
DO $$
BEGIN
  RAISE NOTICE 'QA matrix seed complete. Log in: admin2@naham.com / cook11@naham.demo — password NahamDemo2026!';
END $$;
